...
from utils import spherical_dist_loss, save_progress, TextualInversionDataset
...
def main(args):
    ...
    # assume 49408 max
    index_no_updates = torch.arange(len(tokenizer)) <= 49408
    ...
    for epoch in range(clip_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # create a mask for indexing the EOF token, as Okaris noted, adding new tokens to vocab throws this
            # process off zero out all non-eof tokens
            mask = torch.where(cond_tok_ids == 49408, cond_tok_ids, 0)
